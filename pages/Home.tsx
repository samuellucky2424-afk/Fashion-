  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative h-[70vh] md:h-[85vh] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 responsive-container text-white">
          <p className="uppercase tracking-[0.3em] text-[10px] md:text-sm mb-4 text-amber-400 font-semibold">Excellence in Every Stitch</p>
          <h1 className="text-[2.5rem] md:text-[5rem] lg:text-[7rem] font-serif font-bold leading-[1.1] mb-6 md:mb-8 max-w-4xl">
          ...
          <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
            <Link to="/shop" className="bg-amber-600 hover:bg-amber-700 text-white px-8 md:px-10 py-4 md:py-5 text-xs md:text-sm font-bold uppercase tracking-widest transition-all flex items-center justify-center min-h-[44px]">
      ...
      <section className="py-8 md:py-12 bg-white border-b">
        <div className="responsive-container flex flex-wrap justify-center md:justify-between gap-8 md:gap-4">
          {[
            ...
            <div key={i} className="flex flex-col items-center text-center w-[40%] md:flex-1">
              <div className="w-12 h-12 bg-amber-50 text-amber-600 rounded-full flex items-center justify-center mb-3">
                <badge.icon size={24} />
              </div>
              <h3 className="font-bold text-xs uppercase tracking-wider mb-1">{badge.title}</h3>
              <p className="text-[10px] md:text-xs text-gray-500">{badge.sub}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2">
        <Link to="/shop?category=Men" className="relative group overflow-hidden h-[300px] md:h-[600px]">
          <img
            src="https://images.unsplash.com/photo-1617137968427-85924c800a22?auto=format&fit=crop&q=80&w=1000"
            alt="Men's Fashion"
            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
          />
          <div className="absolute inset-0 bg-black bg-opacity-30 group-hover:bg-opacity-40 transition-all flex flex-col items-center justify-center text-white p-6">
            <h3 className="text-3xl md:text-4xl font-serif font-bold mb-4">Gentlemen</h3>
            <span className="border border-white px-8 py-3 uppercase text-xs tracking-[0.2em] group-hover:bg-white group-hover:text-black transition-all">Explore More</span>
          </div>
        </Link>
        <Link to="/shop?category=Women" className="relative group overflow-hidden h-[300px] md:h-[600px]">
          <img
